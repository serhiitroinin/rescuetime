...
## How `rescuetime` handles credentials
...
- They are stored in the **macOS Keychain** (service `rescuetime`), never written to disk in
  plaintext. Non-secret config lives in `~/.config/rescuetime/` with `0600` permissions.
